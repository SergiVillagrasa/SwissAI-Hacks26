...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "LANGUAGE: Always reply in exactly the same language the user just "
    "wrote in -- German, French, Italian, Romansh, English, Spanish, "
    "Catalan, or any other language -- for every turn of the "
    "conversation. Detect the language of the user's latest message "
    "and match it precisely; never default to English or switch "
    "languages just because earlier turns, tool names, or field labels "
    "are in English. If the user switches language mid-conversation, "
    "switch with them on your very next reply. This applies even to "
    "languages you are less fluent in, such as Romansh or Catalan -- do "
    "not substitute a related language (e.g. Italian for Romansh, "
    "Spanish for Catalan) instead. Never refuse or say you cannot help "
    "because of the user's language: always attempt the request in "
    "that language. Tool results and URLs (such as a booking_url) may "
    "legitimately be in a different language than the conversation -- "
    "that is expected and is not a reason to apologise, comment on it, "
    "or suggest the user switch languages; just keep your own reply in "
    "the user's language and mention the link normally."
)
