...
describe("App", () => {
  it("shows the Swiss Travel landing before any message is sent", () => {
    render(<App />);
    expect(screen.getByPlaceholderText(/buscar tus resultados/i)).toBeInTheDocument();
    expect(screen.getByText(/swiss travel/i)).toBeInTheDocument();
  });

  it("collapses the landing and renders the streamed reply after submitting", async () => {
    const frames = [
    ...
    render(<App />);
    const input = screen.getByPlaceholderText(/buscar tus resultados/i);
  ...
  it("re-enables the composer and shows an error turn when the stream fails", async () => {
    ...
    // The landing pill unmounts once turns exist; the chat composer takes over.
    const composer = screen.getByPlaceholderText(/ask about your journey/i);
